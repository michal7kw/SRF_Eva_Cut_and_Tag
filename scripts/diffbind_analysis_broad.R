...
# Create sample sheet for broad peaks
samples <- data.frame(
  SampleID = c(
    "TES-1", "TES-2", "TES-3",
  ...
  bamControl = c(
    rep("results/04_filtered/IggMs_filtered.bam", 6),
...
write.csv(
  as.data.frame(res_tes_vs_tesmut),
  "results/07_analysis_broad/DiffBind_TES_vs_TESmut.csv"
)
write.csv(
...
# Plot MA - standardized naming
pdf("results/07_analysis_broad/DiffBind_MA_plots.pdf", width = 12, height = 6)
par(mfrow = c(1, 2))
...
# Convert to PNG
png("results/07_analysis_broad/DiffBind_MA_plots.png", width = 12, height = 6, units = "in", res = 300)
par(mfrow = c(1, 2))
dba.plotMA(dba_obj, contrast = 1)
dba.plotMA(dba_obj, contrast = 2)
dev.off()
...
# Venn diagram of overlaps - standardized naming
pdf("results/07_analysis_broad/DiffBind_venn.pdf", width = 8, height = 8)
dba.plotVenn(dba_obj, contrast = c(1, 2))
dev.off()

# Convert to PNG
png("results/07_analysis_broad/DiffBind_venn.png", width = 8, height = 8, units = "in", res = 300)
dba.plotVenn(dba_obj, contrast = c(1, 2))
dev.off()
