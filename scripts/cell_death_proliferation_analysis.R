...
perform_focused_go <- function(gene_ids, sample_name, focus_terms, focus_name) {
    ...
    if (nrow(combined_results) > 0) {
        ...
        # Save plots
        filename_base <- paste0("results/cell_death_proliferation/", sample_name, "_", focus_name, "_GO")
...
perform_kegg_analysis <- function(gene_ids, sample_name) {
    ...
    cancer_keywords <- c("cancer", "carcinoma", "apoptosis", "cell cycle",
    ...
    cancer_pathways <- kegg_result@result[
        grepl(paste(cancer_keywords, collapse = "|"),
              kegg_result@result$Description, ignore.case = TRUE), ]

    if (nrow(cancer_pathways) > 0) {
        # Create visualization
        p1 <- ggplot(head(cancer_pathways, 10),
                     aes(x = Count, y = reorder(Description, Count))) +
            geom_point(aes(color = p.adjust, size = Count)) +
            scale_color_gradient(low = "red", high = "blue", name = "p.adjust") +
            labs(title = paste(sample_name, "Cancer-Related KEGG Pathways"),
                 x = "Gene Count", y = "KEGG Pathways") +
            theme_minimal() +
            theme(axis.text.y = element_text(size = 8))

        # Save results
        filename_base <- paste0("results/cell_death_proliferation/", sample_name, "_KEGG_cancer")
...
perform_reactome_analysis <- function(gene_ids, sample_name) {
    ...
    tryCatch({
        reactome_result <- enrichPathway(gene = gene_ids,
                                       organism = "human",
                                       pAdjustMethod = 'BH',
                                       qvalueCutoff = 0.05,
                                       readable = TRUE)
        ...
        focus_keywords <- c("apoptosis", "death", "proliferation", "cycle",
        ...
        focused_pathways <- reactome_result@result[
            grepl(paste(focus_keywords, collapse = "|"),
                  reactome_result@result$Description, ignore.case = TRUE), ]

        if (nrow(focused_pathways) > 0) {
            # Create visualization
            p1 <- ggplot(head(focused_pathways, 10),
                         aes(x = Count, y = reorder(Description, Count))) +
                geom_point(aes(color = p.adjust, size = Count)) +
                scale_color_gradient(low = "red", high = "blue", name = "p.adjust") +
                labs(title = paste(sample_name, "Cell Death/Proliferation Reactome Pathways"),
                     x = "Gene Count", y = "Reactome Pathways") +
                theme_minimal() +
                theme(axis.text.y = element_text(size = 7))

            # Save results
            filename_base <- paste0("results/cell_death_proliferation/", sample_name, "_Reactome")
...
if (length(gene_lists) >= 2) {
    ...
    if (length(gene_lists) == 2) {
        ...
        venn.diagram(venn_data,
                     filename = "results/cell_death_proliferation/gene_overlap_venn.png",
    ...
    write.csv(overlap_stats, "results/cell_death_proliferation/gene_overlap_stats.csv", row.names = FALSE)
...
write.csv(summary_table, "results/cell_death_proliferation/analysis_summary.csv", row.names = FALSE)
...
cat("\n=== OUTPUT FILES ===\n")
cat("Results saved in: results/cell_death_proliferation/\n")
