...
parse_stats_files <- function(stats_dir = "results/13_ses_overlap") {
  ...
  for (file in stats_files) {
    ...
    # Determine group and replicate
    if (grepl("^TES-[123]$", sample_clean)) {
      group <- "TES"
      replicate <- gsub("TES-", "", sample_clean)
    } else if (grepl("^TESmut-[123]$", sample_clean)) {
      group <- "TESmut"
      replicate <- gsub("TESmut-", "", sample_clean)
...
group_colors <- c("TES" = "#2E86AB", "TESmut" = "#A23B72", "TEAD1" = "#F18F01")
...
summary_text <- paste(
  ...
  "   - TESmut samples show highest overlap percentages across replicates",
  "",
  "Generated plots:",
