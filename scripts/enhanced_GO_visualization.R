...
ego_tes_specific <- enrichGO(gene = tes_entrez$ENTREZID,
                           OrgDb = org.Hs.eg.db,
                           keyType = 'ENTREZID',
                           ont = "BP",
                           pAdjustMethod = "BH",
...
if (!is.null(ego_tes_specific) && nrow(ego_tes_specific@result) > 0) {
    ...
    ego_tead1_specific <- enrichGO(gene = tead1_entrez$ENTREZID,
                                 OrgDb = org.Hs.eg.db,
                                 keyType = 'ENTREZID',
                                 ont = "BP",
                                 pAdjustMethod = "BH",
    ...
    p4 <- ggplot(tes_top_terms, aes(x = NegLog10P, y = Description,
          ...
          xlab("-log10(adjusted p-value)") + ylab("GO Terms")
    ...
    summary_stats <- data.frame(
        Metric = c("Total TES-specific genes", "Genes with ENTREZ ID", "Significant GO terms (p.adj < 0.05)",
                  "Most significant term", "Top enriched process"),
        Value = c(length(tes_specific_genes), nrow(tes_entrez), nrow(ego_tes_specific@result),
                 ego_tes_specific@result$Description[1], ego_tes_specific@result$Description[1]),
        Details = c("Genes found in TES but not TEAD1 peaks", "Successfully converted to ENTREZ",
